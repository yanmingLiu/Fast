...
class FLogEvent {
  ...
  Future<Map<String, dynamic>?> _getCommonParams() async {
    try {
      ...
      final deviceModel = await FService().getDeviceModel();
      final manufacturer = await FService().getDeviceManufacturer();
      final idfv = await FService().getIdfv();
      final version = await FService().version();
      final osVersion = await FService().getOsVersion();
  ...
  Future<void> logInstallEvent() async {
    try {
      ...
      final build = await FService().buildNumber();
      final isLimitAdTrackingEnabled =
          await FService().isLimitAdTrackingEnabled();
      final agent =
  ...
  Future<void> _uploadPendingLogs() async {
    try {
      ...
      final List<EventData> validLogs = [];
      final List<String> invalidLogIds = [];

      for (final logItem in logs) {
        try {
          final data = jsonDecode(logItem.data);
          dataList.add(data);
          validLogs.add(logItem);
        } catch (e) {
          log.e('[ad]log Invalid JSON in log ${logItem.id}: $e');
          invalidLogIds.add(logItem.id);
        }
      }

      if (invalidLogIds.isNotEmpty) {
        try {
          final box = await _adLogService.box;
          await box.deleteAll(invalidLogIds);
          log.w('[ad]log Deleted ${invalidLogIds.length} invalid logs');
        } catch (e) {
          log.e('[ad]log Failed to delete invalid logs: $e');
        }
      }
      ...
      if (res.statusCode == 200) {
        await _adLogService.markLogsAsSuccess(validLogs).timeout(
          const Duration(seconds: 5),
          onTimeout: () {
        ...
        log.d('[ad]log Batch upload success: ${validLogs.length} logs');
      } else {
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              ...
              const PopupMenuItem(
                  value: 'pending', child: Text('Pending Logs')),
